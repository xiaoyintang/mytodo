  ...
  const cardLines = cards.length > 0
    ? cards.map((card) => {
        ...
        const stepsLabel = card.steps?.length
          ? ` · ${card.type === "onetime" ? "任务步骤" : "固定流程"}：${card.steps.map((step, index) => `${index + 1}. ${step.title}`).join(" → ")}`
          : "";
...
3. 在关键结果结构合理之后，再和我一起寻找推进项。请先区分两种关系：互相替代的方案是并列行为备选；必须共同完成才能交付一件事的内容，是一个父任务下面的任务步骤，不要铺平成同级行为。
4. 独立行为必须是某个具体时刻可以开始做的动作；太大的行为请给出最小可执行版本，并把最小版本直接写在同一条行为里。
5. “产出一版答复、完成一份报告、发布一期视频”这类有明确交付物的有限项目，可以作为 [一次性] 任务包，并在同一行附上“任务步骤”。可重复方法则可以附上“固定流程”。两者的步骤都共同完成父项，不互相竞争，也不要分别评价影响力。
...
任务步骤/固定流程是可选的，必须放在该推进项同一行的最后，用“→”给出**完整最终顺序**：
- [新增] [一次性] 产出一版“增长点从哪里来”的标准化答复 | 关键结果：开放性问题表达清楚 | 任务步骤：整理外部数据角度 → 整理内部数据角度 → 写出业务判断逻辑 → 合并成完整答复
...
如果只调整已有推进项的步骤，替换前后可以写同一个父项；App 会只更新流程。省略“任务步骤/固定流程”表示保留已有流程；只有确实要删掉时，一次性任务写“| 任务步骤：清空”，可重复行为写“| 固定流程：清空”。不要把流程中的每一步另写成新的可导入行为。
