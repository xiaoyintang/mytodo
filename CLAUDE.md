...
│   ├── 点击任务主体 → 状态循环
│   ├── 点击「2/6」小按钮 → 展开子任务（**必须自己一个 hit target**，
│   │   否则会被"点主体切状态"抢走）；子任务行点一下就是打勾
...
### 一件事能不能"无脑做"：三个时刻，一行只报一个问题

判断标准统一成一条（`components/todo/blocker.ts`）：

```
① 起点  现在就能动手吗？   → action  还不是行为（type=aspiration/outcome）
                          → timing  没说什么时候做
② 过程  中间会停下来想吗？ → decision 要当场判断/挑选/评估
                          → effort   要动用意志力
③ 终点  做完了我知道吗？   → endpoint 说不出做到哪算完
```

**「卡住」只会发生在这三个时刻，没有第四个**，所以这三条是完备的。

**一行只报最早坏掉的那一个**——修好早的往往顺手把晚的也修了。
愿望本来就没终点，同时喊"这是愿望"和"没终点"只是在吵（用户原话："两个同时触发很尬"）。

各自对应的修复入口：
| 缺口 | 按钮 | 谁判定 |
|---|---|---|
| action | 拆成行为（魔法棒） | AI（type） |
| timing | 改成能无脑做的说法 | AI（只判 habit/stop——一次性任务靠排期给时机） |
| decision / endpoint | 改成能无脑做的说法 | AI（`blocker`） |
| effort | 改小 | **你自己**——费不费力只有你知道，看可行性滑块，不问 AI |

句式模板：**「___ 的时候，做 ___，到 ___ 就算完」**，三个空填满就三个时刻都齐了。

**「改成能无脑做的说法」的铁律：动作不许换，只能加边界。**
原文是"看"就还得是看，是"查"就还得是查——加终点是给它划个范围，不是换一件事去做。
踩过一次坑：prompt 里写了「查面试技巧」→「写出3个最可能被问到的问题」当示例，
结果 AI 把"从外面获取信息"调包成"从自己脑子里掏"，用户原话"货不对版"。
加边界只有两个手法：**限定数量/范围**（一篇、3条、前3段、10分钟）、
**要求留下痕迹**（记在备忘录、划线、截图）。

`blocker` 字段替代了老的 `hasDecision`（保留只为兼容旧数据，重判一次就换掉）。
**判定标准改进后老条目不会自己更新**——焦点地图头部有「重判」，你手动改判过的不动。
...
八环节里福格管下半截，**③明确他基本没写**——「改简历」三个字没法动手，
盯着它发呆不是因为难，是因为它还不是一个"能拍到照片的动作"。

**用子任务列表，不用 GTD 的 `nextAction` 字段**：所谓 nextAction 就是
"只能有一条、还不能打勾的子任务"，是列表的残废版。列表能打勾，于是白得一个进度，
顺带把「穷尽/进度」也解决了。

- **不要求一次拆完**——加一条做一条也行。完整拆解本身是会把人劝退的认知负担
- **打勾直接带动任务状态**：全勾完=done，勾一部分=进行中，全清=待办，反向也成立。
  有子任务时手动进度滑块**自动隐藏**，别让人维护两套账
- AI「拆一下」的 prompt（`BREAKDOWN_PROMPT`）三条铁律：
  ①**穷尽不筛选**——合起来要覆盖整个任务，不许判断哪步更值得做（和焦点地图的取舍正相反）；
  ②每条过照片测试；③**第一条必须是最容易起步的那条**，往往就是"打开某个东西"
- 已经够小的任务（「扔垃圾」）只会拆出它自己一条，此时不写入

**焦点地图是排序工具，不是筛人工具。**所有行为都进地图拿 ROI 排序，
出来的是排好序的清单不是杀掉一半的清单——所以一次性任务**照样上地图**
（「先改简历还是先刷真题」就是 ROI）。分叉在地图**之后**，按可不可重复自动分：
可重复→习惯表→简化/锚点；一次性→日程→拆解。
用户不需要在任何地方选"这个目标是什么型"。

> 踩过的坑：曾经给 Aspiration 加过 `shape`（状态型/项目型）+ 一张 `steps` 表，
> 把分叉放在目标层。错在**一个目标本来就两条腿都要**（考研既要每天背单词，
> 也要做完真题），在目标层分叉等于逼人二选一。已整个 revert。
