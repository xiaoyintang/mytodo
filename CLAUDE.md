...
`hasDecision` 标的是「执行时会卡住」，两种情况：
1. **要当场判断**：「挑出**有问题的**那句去改写」——"有问题的"得现场评估
2. **没有完成条件**：「**查一下**面试技巧」——查到什么程度算完？可以查 10 分钟也可以查 3 小时。
   典型词：查一下 / 了解一下 / 研究一下 / 看看 / 多 X 一点
两者的治法一样（改写成有明确终点或产出物的说法），所以共用一个字段，不新加。
行上给「改成有终点的说法」按钮 → `/api/behavior (mode:concrete)`，和「改小」共用一套候选 UI：
- **改小**（做不到）：小到不需要意志力
- **改具体**（会卡住）：做完了自己一眼就知道——有终点或有产出物
  手法是"把判断换成数出来、把查一下换成写下来、一句话塞好几步的只留第一步"

**判定标准（prompt）改进之后，老条目不会自己更新**——`hasDecision`/`type` 只在判定那一刻写一次。
目标详情页顶上有「全部重判」，你手动改判过的不动。
第 2 类是"看起来像干活的拖延"最常见的形态，对目标推进的边际收益衰减极快，
在焦点地图上会落到右下角（好做但没用），本来就不该占格子。
